...
class Planner():
    ...
    def extract_plan(self, state, parents):
        ...
        while parents[state] != None:
            prev_state, option = parents[state]
            route = [(prev_state, option)] + route
            state = self.to_tuple(prev_state)
        return route
            ...
            for state in option.list_initiation_states():
                if a_subset_b(S, self.N(state)):
                    ...
                    if self.to_tuple(term_state) not in reached:
                        sucessors.append([term_state, option])
        ...
        while (not frontier.empty()):
            ...
            for i in self.get_sucessors(state):
                ...
                if a_subset_b(i[0], self.N(G)):
                    parents[self.to_tuple(i[0])] = [state, i[1]]
...
if __name__ == "__main__":
    ...
    planner = Planner(mdp_2, neigh.N2)
    arr1 = np.zeros((8, 8))
    arr1[1, 0] = 1
    arr2 = np.zeros((8, 8))
    arr2[2, 6] = 1
